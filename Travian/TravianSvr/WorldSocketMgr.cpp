...
class ReactorRunnable : protected ACE_Task_Base
{
    ...
    protected:
        ...
        virtual int svc ()
        {
            ...
            printf("Network Thread Exitting\n");
...
WorldSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
    ...
    if (num_threads <= 0)
    {
        printf("Network.Threads is wrong in your config file\n");
        return -1;
    }
    ...
    if ( m_SockOutUBuff <= 0 )
    {
        printf("Network.OutUBuff is wrong in your config file\n");
        return -1;
    }
...
WorldSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            printf("WorldSocketMgr::OnSocketOpen set_option SO_SNDBUF\n");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer ().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            printf("WorldSocketMgr::OnSocketOpen: peer ().set_option TCP_NODELAY errno = %s\n", ACE_OS::strerror (errno));
            return -1;
        }
    }
	...
	if (sock->peer ().get_remote_addr (remote_addr) == -1)
	{
	...
	{
		printf("WorldSocketMgr add socket (%s:%d) into Reactor [%d]\n", 
			remote_addr.get_host_addr (), remote_addr.get_port_number(), min);
	}
